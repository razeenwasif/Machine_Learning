...
def search_C(X_train, y_train, X_val, y_val, return_best_acc=False):
    ...
    # NOTE: Why not concatenating X_train and X_val, then using GridSearchCV from sklearn?
    #       Because the TF-IDF features generated by TfidfVectorizer depends on the data used to fit TfidfVectorizer,
    #       so to correctly use GridSearchCV, a different TfidfVectorizer needs to be fit using the corresponding 
    #       training split (which are list of document strings) in each iteration of the cross validation.
    #       The code structure for this question is not designed to support this (on purpose).

    print('Searching best hyper parameter (C) value ...')

    C_candidates = [0.01, 0.1, 1, 10, 100, 1000]
    best_C = 1.  # sklearn default
    best_acc = -1.0 

    for C_val in C_candidates:
        model = train_model(X_train, y_train, C=C_val)
        current_acc = eval_model(X_val, y_val, model)

        print(f"  C = {C_val:<6} | Validation accuracy = {current_acc:.4f}")

        if current_acc > best_acc:
            best_acc = current_acc 
            best_C = C_val 

    print(f"\nFound best C = {best_C} with validation accuracy = {best_acc:.4f}")
